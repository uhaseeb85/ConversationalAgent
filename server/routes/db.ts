...
// POST /api/db/execute
// SECURITY: This endpoint executes SQL generated by the app - NOT arbitrary user queries
// SQL is generated by src/lib/sql-generator.ts from validated form submissions
router.post('/execute', async (req: AuthRequest, res: Response) => {
  ...
  // SECURITY: Validate that statements only contain allowed operations
  const allowedOps = /^\s*(INSERT|UPDATE|DELETE)\s+/i
  for (const stmt of statements) {
    if (!allowedOps.test(stmt.trim())) {
      res.status(403).json({ 
        ok: false, 
